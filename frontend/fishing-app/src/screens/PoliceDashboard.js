...
import { getUserInfo, labelFor } from "../data/policeUserDirectory";
import {
  getLatestBoats,
  getActiveAlerts,
  getViolationReports,
...
const buildFocusedFromMap = (b, alerts) => {
  const a = alerts.find(
    (x) =>
      (x.boatId || x.reporterId) === b.boatId &&
      (x.status || "").toLowerCase() === "active"
  );
  return { ...b, alertId: a?._id, status: a ? "sos" : b.status };
};

function SectionTitle({ children }) {
  return (
    <Text className="text-blue font-bold text-lg px-4 mt-2 mb-2">
      {children}
    </Text>
  );
}
...
export default function PoliceDashboard() {
  ...
  const [sosUiStatus, setSosUiStatus] = useState({});

  // 🆕 hydrate on mount
  useEffect(() => {
    (async () => {
      try {
        const saved = await AsyncStorage.getItem("sosUiStatus");
        if (saved) setSosUiStatus(JSON.parse(saved));
      } catch {}
    })();
  }, []);
  ...
  useEffect(() => {
    AsyncStorage.setItem("sosUiStatus", JSON.stringify(sosUiStatus));
  }, [sosUiStatus]);
 ...
 lng: toNum(a.location?.longitude),
  ...
  const sosWithLocal = useMemo(
    () =>
      sosList.map((i) => ({
        ...i,
        //localStatus: sosUiStatus[i.alertId] || "active",
        localStatus: sosUiStatus[i.boatId] || "active",
      })),
    [sosList, sosUiStatus]
  );
  ...
  const filteredSos = useMemo(
    () =>
      sosFilter === "all"
        ? sosWithLocal
        : sosWithLocal.filter((i) => i.localStatus === sosFilter),
    [sosWithLocal, sosFilter]
  );
      ...
      const normalized = raw
        .map((row) => {
          ...
          return {
            ...
            lat: toNum(source?.latitude ?? row.latitude),
            lng: toNum(source?.longitude ?? row.longitude),
    ...
    if (tab === "reports") {
      (async () => {
        ...
        try {
          ...
          setHazards(
            Array.isArray(h?.data) ? h.data : Array.isArray(h) ? h : []
          );
        } catch (e) {
  ...
  function TabButton({ value, label }) {
    ...
    return (
      ...
      >
        <Text
          className={`${active ? "text-white" : "text-blue"} font-semibold`}
        >
          {label}
        </Text>
      </TouchableOpacity>
  ...
  function FilterChip({ value, current, onChange, label }) {
    ...
    return (
      ...
      >
        <Text
          className={`${active ? "text-white" : "text-blue"} text-xs font-semibold`}
        >
          {label}
        </Text>
      </TouchableOpacity>
  ...
  return (
    <SafeAreaView className="flex-1 bg-white" edges={["top"]}>
        ...
        <View className="flex-1">
          ...
          {loadingAlerts ? (
            <View className="flex-1 justify-center items-center">
              ...
              <Text className="text-darkBlue font-sans mt-4">
                Loading emergency alerts...
              </Text>
            </View>
          ) : (
            <>
              {/* 🆕 SOS status filter row */}
              <View className="flex-row px-6 mb-2">
                {[
                  ...
                  <TouchableOpacity
                    key={v}
                    onPress={() => setSosFilter(v)}
                    className={`px-3 py-1.5 mr-2 rounded-full border ${sosFilter === v ? "bg-blueLight border-blueLight" : "border-blueLight"}`}
                  >
                    <Text
                      className={`${sosFilter === v ? "text-white" : "text-blue"} text-xs font-semibold`}
                    >
                      {v === "all"
                        ? "All"
                        : v
                ...
                ListEmptyComponent={
                  <View className="items-center mt-16">
                    <View className="bg-green-50 rounded-3xl p-8 items-center border border-green-200">
                      ...
                      <Text className="text-darkBlue font-heading font-bold text-xl mb-2">
                        All Clear!
                      </Text>
                  ...
                  return (
                    ...
                    >
                      ...
                      <View className="flex-row items-center justify-between">
                        {/* 🆕 dynamic chip */}
                        <View className={`${chip.bg} rounded-full px-4 py-2`}>
                          <Text className="text-white text-sm font-sans font-bold">
                            {chip.label}
                          </Text>
                        </View>
                        <Text className="text-accentText font-sans font-semibold">
                          View on Map →
                        </Text>
                      </View>
                    </TouchableOpacity>
      ...
      {tab === "map" && (
        <View
          className="flex-1 mx-6 mb-6 mt-4 rounded-3xl overflow-hidden border-2 border-lightPurple shadow-2xl"
          style={{
            shadowColor: "#636CCB",
        ...
        >
            ...
            {boats.map((b) => {
              ...
              return (
                <Marker
                  ...
                  onPress={() => setFocusedBoat(buildFocusedFromMap(b, alerts))}
                >
                  {isSOS && (
                  ...
                  <Callout
                    onPress={() =>
                      setFocusedBoat(buildFocusedFromMap(b, alerts))
                    }
                  >
                    <View style={{ maxWidth: 240, padding: 12 }}>
                      ...
                      <Text
                        style={{ color: "#6E8CFB", marginTop: 8, fontSize: 14 }}
                      >
                        Tap for details →
                      </Text>
                    </View>
          ...
          >
            <View className="flex-row items-center">
              ...
              <Text className="ml-2 text-blue font-sans font-semibold">
                Fit All Boats
              </Text>
            </View>
          </TouchableOpacity>
                ...
                <View className="mb-8">
                  ...
                  {filteredViolations.length === 0 ? (
                    ...
                    filteredViolations.map((r) => (
                      <View
                        key={r._id}
                        className="bg-white rounded-3xl p-6 mb-4 border border-lightPurple shadow-xl"
                        style={{
                          shadowColor: "#000",
                          shadowOffset: { width: 0, height: 6 },
                          shadowOpacity: 0.1,
                      ...
                      >
                        <View className="flex-row justify-between items-start mb-3">
                          ...
                          >
                            <Text
                              className={`text-xs font-sans font-semibold capitalize ${
                                (r.status || "").toLowerCase() === "verified"
                                  ? "text-green-800"
                          ...
                          >
                            {r.evidence.imageUrl.map((u, i) => (
                              <Image
                                key={i}
                                source={{ uri: u }}
                                className="w-28 h-28 rounded-2xl mr-3 border border-lightPurple"
                              />
                            ))}
                          </ScrollView>
                        )}
      ...
      >
        <View className="flex-1 justify-end bg-black/60">
          <View
            className="bg-white rounded-t-3xl p-6 mx-2 mb-2 shadow-3xl border border-lightPurple"
            style={{
              shadowColor: "#000",
